...
import (
	...
	"GinSkeletonLearn/app/service/websocket/on_open_success"
	"errors"
...
func (c *Client) OnOpen(context *gin.Context) (*Client, bool) {
	...
	//2. 将http协议升级成websocket协议，初始化一个有效的websocket长链接客户端
...
// 实时接受数据
func (c *Client) ReadPump(callBackMessage func(messageType int, receivedData []byte), callBackOnError func(err error), callBackOnClose func()) {
	//回调onclose事件
	defer func() {
		if err := recover(); err != nil {
			if val, ok := err.(error); ok {
				variable.ZapLog.Error(my_errors.ErrorsWebsocketReadMessageFail, zap.Error(val))
			}
		}
	}()
	...
	for {
		if c.State == 1 {
			messageType, bReceivedData, err := c.Conn.ReadMessage()
			if err != nil {
				callBackMessage(messageType, bReceivedData)
			} else {
				callBackOnError(err)
				break
			}
	...
	defer func() {
		...
		if err != nil {
			if err, ok := err.(error); ok {
				variable.ZapLog.Error(variable.WebsocketServerPingMsg, zap.Error(err))
			}
		}
