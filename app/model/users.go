...
import (
	...
	"GinSkeletonLearn/app/utils/md5_encrypt"
	"go.uber.org/zap"
	"time"
)
...
// 用户登录
func (u *UsersModel) UserLogin(userName string, pass string) *UsersModel {
	sql := "select id ,user_name,pass,phone,real_name from tb_users where user_name=? limit 1"
...
func (u *UsersModel) OauthLoginToken(userId int64, token string, expiresAt int64, clientIp string) bool {
	sql := `INSERT INTO tb_oauth_access_token(fr_user_id,action_name,token,expires_at,client_ip)
			select ?,'login',?,?,? from dual where NOT exists(select 1 from tb_oauth_access_token a where a.fr_user_id=? and a.action_name='login'anda.token=?)`
