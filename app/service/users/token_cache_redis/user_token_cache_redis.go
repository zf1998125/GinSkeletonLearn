...
import (
	...
	"GinSkeletonLearn/app/utils/redis_factory"
	"go.uber.org/zap"
	"strconv"
	"strings"
	"time"
...
func CreateUsersTokenCacheFactory(userId int64) *userTokenCacheRedis {
	...
	return &userTokenCacheRedis{redisClient: redisClient, userTokenKey: "token_userid_" + strconv.FormatInt(userId, 10)}
}

// SetTokenCache 设置缓存
func (u *userTokenCacheRedis) SetTokenCache(tokenExpire int64, token string) bool {
	// 存储用户token时转为MD5，下一步比较的时候可以更加快速地比较是否一致
	if _, err := u.redisClient.Execute("zAdd", u.userTokenKey, tokenExpire, md5_encrypt.MD5(token)); err == nil {
		return true
	} else {
