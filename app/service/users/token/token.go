...
import (
	...
	"GinSkeletonLearn/app/model"
	"GinSkeletonLearn/app/service/users/token_cache_redis"
	"errors"
...
func (user *userToken) GenerateToken(userId int64, username, phone string, expireAt int64) (tokens string, err error) {
	...
	return user.userJwt.CreateToken(customClaims)
}

// RecordLoginToken 用户login成功，记录用户token
func (u *userToken) RecordLoginToken(token string, clientIp string) bool {
	customClaims, err := u.userJwt.ParseToken(token)
	if err != nil {
		return false
	} else {
		userId := customClaims.UserId
		expiresAt := customClaims.ExpiresAt
		return model.CreateUserFactory("").OauthLoginToken(userId, token, expiresAt, clientIp)
	}
}
...
func (u *userToken) DestroyToken(token string) bool {
	parseToken, err := u.userJwt.ParseToken(token)
	if err != nil {
		return false
	} else {
		userId := parseToken.UserId
		model.CreateUserFactory("").OauthDestroyToken(int(userId))
		return true
	}
