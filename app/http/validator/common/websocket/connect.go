...
import (
	...
	"GinSkeletonLearn/app/http/controller/websocket"
...
func (c Connect) CheckParams(context *gin.Context) {
	...
	if extraAddBindDataContext == nil {
		...
		if seviceWs, ok := (&websocket.Ws{}).OnOpen(extraAddBindDataContext); ok == false {
			variable.ZapLog.Error(consts.WsOpenFailMsg)
		} else {
			(&websocket.Ws{}).OnMessage(seviceWs, extraAddBindDataContext) // 注意这里传递的service_ws必须是调用open返回的，必须保证的ws对象的一致性
		}
	}
}
