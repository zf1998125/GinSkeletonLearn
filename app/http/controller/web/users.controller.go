...
import (
	...
	"GinSkeletonLearn/app/service/users/curd"
	"GinSkeletonLearn/app/service/users/token"
...
func (u *Users) Login(context *gin.Context) {
	...
	userModel := userModelFact.UserLogin(userName, pass)
	if userModel != nil {
...
func (u *Users) RefreshToken(context *gin.Context) {
	...
	if newToken, ok := token.CreateUserTokenFactory().RefreshToken(oldToken, context.ClientIP()); ok {
		res := gin.H{
			"token": newToken,
		}
